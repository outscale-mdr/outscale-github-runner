...
sed -ri "s|myoung34/github-runner:${CURRENT_VERSION}-ubuntu-bionic|myoung34/github-runner:${UPGRADE_VERSION}-ubuntu-bionic|g" "${root}/terraform/env_file/etc/systemd/system/github-runners.service"
...
result=$(curl -s -X POST -H "Authorization: token $GH_TOKEN" -d "{\"head\":\"$branch_name\",\"base\":\"main\",\"title\":\"Upgrade Github Runner ($UPGRADE_VERSION)\",\"body\":\"Automatic upgrade of Github Runner to $UPGRADE_VERSION. **Changelog**: https://github.com/myoung34/docker-github-actions-runner/compare/$CURRENT_VERSION...$UPGRADE_VERSION\"}" "https://api.github.com/repos/outscale-dev/outscale-github-runner/pulls")
