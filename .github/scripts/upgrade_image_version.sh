...
result=$(curl -s -X POST -H "Authorization: token $GH_TOKEN" -d "{\"head\":\"$branch_name\",\"base\":\"main\",\"title\":\"Upgrade Github Runner ($UPGRADE_VERSION)\",\"body\":\"Automatic upgrade of Github Runner to $UPGRADE_VERSION\"}" "https://api.github.com/repos/outscale-dev/outscale-github-runner/pulls")
